...
function initSearchFunctionality() {
  ...
  document.addEventListener('click', (e) => {
    if (!DOM.searchForm.contains(e.target) && !DOM.searchHistory.contains(e.target)) {
...
function updateSearchHistoryUI() {
  ...
  if (searchHistory.length === 0) {
    ...
    searchHistory.forEach((item, index) => {
      ...
      historyItem.className = 'px-3 py-2 hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer text-left';
...
function initFavoritesFunctionality() {
  ...
  DOM.favoritesBar.addEventListener('mouseleave', () => {
    ...
    setTimeout(() => {
      if (!DOM.favoritesBar.matches(':hover') && !DOM.addFavoriteBtn.matches(':hover')) {
...
function updateFavoritesUI() {
  ...
  favorites.forEach((favorite, index) => {
    ...
    favoriteBtn.className = 'relative p-2 transition-all duration-300 hover:bg-primary/10 rounded-lg';
    ...
    if (favorite.icon && favorite.icon.startsWith('data:image/')) {
      ...
      // 默认图标
      iconHTML = `<div class="text-primary mb-1"><i class="fa ${favorite.icon || 'fa-globe'} text-xl"></i></div>`;
    }
  ...
  addBtn.className = 'relative p-2 transition-all duration-300 hover:bg-primary/10 rounded-lg opacity-0';
  addBtn.innerHTML = `
    <div class="flex flex-col items-center justify-center w-full">
...
function closeSettingsMenuOnClickOutside(event) {
  if (!DOM.settingsBtn.contains(event.target) && !DOM.settingsMenu.contains(event.target) && !DOM.settingsContainer.contains(event.target)) {
...
function handleFileSelection(event) {
  ...
  reader.onload = function(e) {
    ...
    customWallpaperElement.className = 'fixed inset-0 z-0 bg-cover bg-center custom-wallpaper';
    ...
    glassEffect.className = 'fixed inset-0 z-5 bg-black/40 backdrop-blur-sm custom-wallpaper';
